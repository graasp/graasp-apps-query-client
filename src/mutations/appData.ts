...
export default (queryConfig: QueryClientConfig) => {
  ...
  const usePostAppData = () => {
    ...
    return useMutation({
      ...
      onSettled: () => {
        if (!enableWebsocket) {
          queryClient.invalidateQueries({ queryKey: appDataKeys.allSingles() });
        }
      },
    });
  };
  ...
  const usePatchAppData = () => {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: (payload: Partial<Pick<AppData, 'visibility' | 'data'>> & { id: UUID }) => {
        const apiHost = getApiHost(queryClient);
        const data = getDataOrThrow(queryClient);
  ...
  const useDeleteAppData = () => {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: (payload: { id: AppData['id'] }) => {
        const apiHost = getApiHost(queryClient);
        const data = getDataOrThrow(queryClient);
  ...
  const useUploadAppDataFile = () => {
    const queryClient = useQueryClient();
    return useMutation({
      mutationFn: async ({ error }: { error?: Error; data?: unknown }) => {
